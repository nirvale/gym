...
class Clientes extends Controller
{
    ...
    public function pdfPago($id)
    {
        ...
        $pdf = new PDF_HTML('P', 'mm', array(80, 150));
        $pdf->AddPage();
        ...
        $pdf->Cell(40, 5, 'RFC: ', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(40, 5, $empresa['ruc'], 0, 1, 'L');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(40, 5, utf8_decode('TEL: '), 0, 0, 'R');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(40, 5, $empresa['telefono'], 0, 1, 'L');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(40, 5, utf8_decode('DIR: '), 0, 0, 'R');
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell(40, 5, utf8_decode($empresa['direccion']), 0, 'L');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(40, 5, 'DIA: ', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(40, 5, $data['fecha'], 0, 1, 'L');
        //Encabezado
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(72, 10, 'DATOS DEL SOCIO', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(78, 5, 'Ticket: ' . $data['dni'], 0, 1, 'L');
        $pdf->MultiCell(78, 5, 'Socio: ' . utf8_decode($data['nombre']), 0, 'L');
        $pdf->Cell(78, 5, utf8_decode('Direccion: ' . $data['direccion']), 0, 1, 'L');
        $pdf->MultiCell(78, 5, utf8_decode('Membresía: ' . $data['plan']), 0, 'L');
